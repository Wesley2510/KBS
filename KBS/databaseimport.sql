...
CREATE TABLE IF NOT EXISTS `Textbug`.`Menuitem` (
  ...
  INDEX `fk_Menuitem_Bericht1_idx` (`pagina` ASC),
  CONSTRAINT `fk_Menuitem_Bericht1`
    FOREIGN KEY (`pagina`)
    REFERENCES `Textbug`.`Bericht` (`pagina`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
