...
$inputEmailCode = filter_input(INPUT_GET, "e"); //Email bij resetopdracht
...
<html>
    ...
    if (!$succes) {
        ...
        if (!$succesPass || $inputResetPass == NULL) {
            echo "<form id='resetForm' action='#' method='post'></form>";
        echo "<h4>Vul een nieuw wachtwoord in</h4>";
        echo "<input type='password' name='resetPass'";
        if ($resetPassError > 0) {
            echo "class='error'";
        }
        echo "/>";
        echo "<h4>Herhaal</h4>";
        echo "<input type='password' name='resetPassRepeat'";
